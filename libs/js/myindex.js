...
function createCard(id, nameTag, fullName, email, location, department, jobTitle) {
  const new_id = `${id}`;
  const modaltest=$(`<button id="${new_id}-card" type="button" class="btn btn-primary card-btn" data-toggle="modal" data-target="#exampleModalLong"> </button>`);
...
function getSearchedEmployeesIdByName(uname){
  ...
  var nameTag;
    for(i=0; i < allEmployees.length; i++){
      nameTag = `${allEmployees[i].id}-card`;
